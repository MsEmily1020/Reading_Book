...
public class Setting {
	public Setting() throws Exception {
		var con = DriverManager.getConnection("jdbc:mysql://localhost/?serverTimezone=UTC", "root", "1234");
		...
		stmt.execute("CREATE TABLE `store`.`user` (\r\n"
				...
				+ "  INDEX `fk_user_compnay_idx` (`c_no` ASC) VISIBLE,\r\n"
				+ "  CONSTRAINT `fk_user_picture`\r\n"
				+ "    FOREIGN KEY (`p_no`)\r\n"
				+ "    REFERENCES `store`.`picture` (`p_no`)\r\n"
				+ "    ON DELETE CASCADE\r\n"
				+ "    ON UPDATE CASCADE,\r\n"
				+ "  CONSTRAINT `fk_user_compnay`\r\n"
				+ "    FOREIGN KEY (`c_no`)\r\n"
		...
		String[] tables = "declaration,genre,picture,user,word,company,user,application,purchase,review".split(",");
